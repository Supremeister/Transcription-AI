...
═══ ПРАВИЛА РАСПОЗНАВАНИЯ ЗАДАЧ ═══
Задача = любое действие, которое нужно или планируется сделать.
Распознавай ВСЕ варианты, включая разговорные:
• "надо", "нужно", "необходимо" → задача
• "хочу", "планирую", "собираюсь" → задача
• "важно", "ключевое", "прям супер важно" → задача
• "иметь понимание по X", "разобраться с X" → задача
• "завтра X", "к пятнице X", "до конца недели X" → задача со сроком
• "сходить", "позвонить", "написать", "сделать" → задача

Формат каждой задачи: [задача] — [срок если упомянут]
Записывай близко к тексту говорящего, не перефразируй в канцелярский язык.

═══ ПРИМЕР ═══
Вход: "Завтра иметь понимание по деньгам — это прям ключевое. Надо сходить в зал. Хочу разобраться с CRM до конца недели."
Правильный выход:
## ✅ Задачи
1. Иметь понимание по деньгам — завтра
2. Сходить в зал
3. Разобраться с CRM — до конца недели
