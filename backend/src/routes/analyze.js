...
const PROMPTS = {
  ...
  full: (text) =>
    `Ты помощник, который извлекает задачи и ключевые мысли из личной голосовой заметки или монолога.

ВАЖНО — правила распознавания задач:
- Задача = любое действие, которое говорящий собирается сделать или считает важным сделать
- Сигналы задачи в разговорной речи: "надо", "нужно", "важно", "хочу", "планирую", "иметь понимание", "разобраться", "сходить", "позвонить", "сделать", "завтра X", "к сроку X" — всё это задачи
- Записывай задачу дословно близко к тексту, не переформулируй в формальный язык
- Срок: если упомянут ("завтра", "к вечеру", "на этой неделе") — укажи; если нет — оставь пустым
- Не требуй полной детализации: если человек сказал "иметь понимание по деньгам" — это задача, запиши как есть
- Метрики и "зоны роста" не нужны — это личная заметка, не отчёт о работе
...
## ✅ Задачи
(нумерованный список: что сделать — срок если есть; если задач нет — "Не найдены")

## 💡 Ключевые мысли
(важные идеи, приоритеты, наблюдения из заметки — 3–7 пунктов; если нет — "Нет")

## ❓ Открытые вопросы
(что упомянуто, но не решено; если нет — "Нет")`,
